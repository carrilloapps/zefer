...
const MODES: { key: KeygenMode; labelKey: string }[] = [
    ...
    <PageLayout>
      ...
      <div className="glass glow-green-sm p-6 sm:p-8 mb-6 animate-in">
            ...
            <div className="flex gap-0.5 glass !rounded-lg p-0.5" role="group" aria-label={t("gen.mode")}>
              {MODES.map((m) => (
                <button
                  ...
                  className={`flex-1 py-2 text-[10px] font-medium rounded-md chip-select cursor-pointer ${
        ...
        <button type="button" onClick={generate} className="btn-primary">
          ...
          {t("gen.generate")}
          <span className="font-mono text-[10px] opacity-80">~{bits} {t("gen.metric.bits")}</span>
        </button>
        ...
        <div className="mt-5">
          {keys.length === 0 ? (
            ...
            <>
              ...
              <ul key={genTick} className="space-y-1.5 stagger-children">
                {keys.map((k, i) => (
                  <li key={`${genTick}-${i}`} className="glass !rounded-lg px-3 py-2 flex items-center gap-2 animate-in">
                    <code className="flex-1 text-[11px] font-mono theme-text break-all select-all leading-relaxed">{k}</code>
                    <button
                      type="button"
                      onClick={() => copyOne(k, i)}
      ...
      {/* ─── Analyzer ─── */}
      <div className="glass p-6 sm:p-8 animate-in">
